    ...
    return nowPlayingMovies.length > 0 ? (
            ...
            <div className="overflow-x-auto pb-4">
                <div className="group flex flex-wrap gap-4 mt-4 w-max">
                    {nowPlayingMovies.map((movie) => (
                        <div
                            key={movie.id}
                            onClick={() => setSelectedMovie(movie.id)} // Changed _id to id to match the check below
                            className={`relative max-w-40 cursor-pointer transition duration-300 hover:-translate-y-1 ${selectedMovie === movie.id ? 'ring-2 ring-primary rounded-lg' : 'opacity-80 hover:opacity-100'}`}
                        >
        ...
        <div className="mt-8">
            ...
            <div className="inline-flex items-center gap-2 border border-gray-600 px-3 py-2 rounded-md">
                ...
                <input min={0} type="number" value={showPrice} on Change={(e) => setShowPrice(e.target.value)} placeholder="Enter show price" className="outline-none" />
            </div>
        </div>
